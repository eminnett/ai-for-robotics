...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        alpha = min(motion[0], max_steering_angle)
        d = motion[1]
        ...
        horiz_error = random.uniform(-self.distance_noise, self.distance_noise)
        x += horiz_error

        vert_error = random.uniform(-self.distance_noise, self.distance_noise)
        y += vert_error

        steering_error = random.uniform(-self.steering_noise, self.steering_noise)
        theta = (self.orientation + beta + steering_error) % (2 * pi)
    ...
    # according to
    #           self.bearing_noise
    def sense(self, include_noise=1): #do not change the name of this function
        Z = []
        ...
        for L in landmarks:
            dx = L[1] - self.x
            dy = L[0] - self.y
            bearing_error = random.uniform(-self.bearing_noise, self.bearing_noise)
            bearing = (atan2(dy, dx) - self.orientation + bearing_error)
            bearing %= (2 * pi)
            Z.append(bearing)
