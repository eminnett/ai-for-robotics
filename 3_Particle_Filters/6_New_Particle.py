...
for i in range(N):
    probability_ranges.append((pointer, pointer + w[i] / sum_of_weights))
    pointer += w[i] / sum_of_weights
for i in range(N):
    rand = random.uniform(0, 1)
    for j in xrange(len(probability_ranges)):
        range = probability_ranges[j]
        if range[0] < rand and range[1] >= rand:
            p3.append(p2[j])
            break
