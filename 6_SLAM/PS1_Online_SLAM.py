...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        assignment_list = [0,1] + range(4, dim+2)
        Omega = Omega.expand(dim+2, dim+2, assignment_list)
        Xi = Xi.expand(dim+2, 1, assignment_list, [0])
        ...
        for b in range(2):
            Omega.value[b  ][n+b] += -1.0 / motion_noise
            Omega.value[n+b][b  ] += -1.0 / motion_noise
