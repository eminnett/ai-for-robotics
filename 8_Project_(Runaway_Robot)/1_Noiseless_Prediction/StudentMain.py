...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER == None:
        OTHER = {}
        OTHER['previous_xs'] = [measurement[0]]
        OTHER['previous_ys'] = [measurement[1]]

    dx = measurement[0] - OTHER['previous_xs'][-1]
    dy = measurement[1] - OTHER['previous_ys'][-1]
    ...
    if len(OTHER['previous_xs']) > 1:
        dx2 = OTHER['previous_xs'][-1] - OTHER['previous_xs'][-2]
        dy2 = OTHER['previous_ys'][-1] - OTHER['previous_ys'][-2]
        dot_product = dx*dx2 + dy*dy2
        normalised_dot_product = dot_product / dist**2
        if normalised_dot_product < 1 and normalised_dot_product > -1:
            theta = acos(normalised_dot_product)
        alpha = atan2(dy,dx)

    angle = alpha + theta
    xy_estimate = (measurement[0] + dist * cos(angle), measurement[1] + dist * sin(angle))

    OTHER['previous_xs'].append(measurement[0])
    OTHER['previous_ys'].append(measurement[1])
...
# represents the measured position of the broken robot, the blue arrow represents
# your prediction for where the broken robot will be, and the green turtle
...
def demo_grading(estimate_next_pos_fcn, target_bot, OTHER = None):
    ...
    while not localized and ctr <= 10:
        ...
        # print true_position
        print error, distance_tolerance
        if error <= distance_tolerance:
            print "You got it right! It took you ", ctr, " steps to localize."
            localized = True
        if ctr == 10:
